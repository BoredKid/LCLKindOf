...
const addMember = (seq, newMember) => seq(val => next => pair(val)(addMember(next,newMember)),val => pair(val)(fPair(newMember)));

const reverse = seq => seq(val => next => addMember(reverse(next), val),val => fPair(val));

// for testing purpose
const jsPairToArray = pair => pair(val => next => [val, ...jsPairToArray(next)] ,val => [val]);
