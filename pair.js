...
const range = (fst, snd) =>
  fst === snd ? fPair(fst) : pair(fst)(range(fst + 1, snd));

const foreach = (seq, fn) => seq(val => next => foreach(next, fn, fn(val)), fn);

const map = (seq, fn) =>
  seq(val => next => pair(fn(val))(map(next, fn)), val => fPair(fn(val)));

const addMember = (seq, newMember) =>
  seq(
...
const reverse = seq =>
  seq(val => next => addMember(reverse(next), val), val => fPair(val));

// for testing purpose
const jsPairToArray = pair =>
  pair(val => next => [val, ...jsPairToArray(next)], val => [val]);
