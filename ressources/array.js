...
// Array
const createSimpleArray = (n, fn = () => 0, acc = dico.fDictionnary) => (
  n > 0
...
const addToArray = newMember => array => dico.addMember(
  getLargerIndex(array) + 1, newMember,
)(array);

const createFullArrayFromAcc = acc => val => (
  val
    ? createFullArrayFromAcc(addToArray(val)(acc))
    : acc
);
...
const getLowerIndex = (array, prevIndex = -1) => array(
  index => next => getLowerIndex(next, prevIndex < index && prevIndex >= 0 ? prevIndex : index),
  () => prevIndex,
);
...
const reduce = array => (fn, acc) => array(
  (_, val) => next => reduce(next)(fn, fn(acc, val)),
  () => acc,
);
...
const deleteNthLastElements = num => array => (
  num > 0
    ? deleteNthLastElements(num - 1)(deleteByIndex(getLargerIndex(array))(array))
    : array);
...
) => deleteNthFirstElements(
  firstIndex - getLowerIndex(array),
)(deleteNthLastElements(getLargerIndex(array) - lastIndex)(array));

const affectAtIndex = index => value => array => dico.addMember(index, value)(
  deleteByIndex(index)(array),
);
